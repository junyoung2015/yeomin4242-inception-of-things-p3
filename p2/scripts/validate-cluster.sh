...
vagrant ssh yeominS -c "kubectl wait --for=condition=Ready node/yeominS --timeout=180s"

echo "=== Checking deployments, replicas, services, endpoints, and Ingress ==="
vagrant ssh yeominS -c "kubectl get deployments app-1 app-2 app-3 -o wide"
vagrant ssh yeominS -c "kubectl rollout status deployment/app-1 --timeout=180s"
vagrant ssh yeominS -c "kubectl rollout status deployment/app-2 --timeout=180s"
vagrant ssh yeominS -c "kubectl rollout status deployment/app-3 --timeout=180s"
vagrant ssh yeominS -c "test \"\$(kubectl get deployment app-2 -o jsonpath='{.spec.replicas}')\" = 3"
vagrant ssh yeominS -c "test \"\$(kubectl get deployment app-2 -o jsonpath='{.status.readyReplicas}')\" = 3"
vagrant ssh yeominS -c "kubectl get svc app-1 app-2 app-3"
vagrant ssh yeominS -c "kubectl get endpoints app-1 app-2 app-3"
vagrant ssh yeominS -c "kubectl get ingress multi-app-ingress"
vagrant ssh yeominS -c "kubectl get pods -n kube-system -l app.kubernetes.io/name=traefik"
